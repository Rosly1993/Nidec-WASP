...
var rpaPlanCount = table.rows().data().filter(function (row) {
    return row.customized_level2_status === 'Plan' && row.type === 'SYSTEM';
}).length;

var rpaAcceptCount = table.rows().data().filter(function (row) {
    return row.customized_level2_status === 'Accept' && row.type === 'SYSTEM';
}).length;

var rpaOngoingCount = table.rows().data().filter(function (row) {
    return row.customized_level2_status === 'Ongoing' && row.type === 'SYSTEM';
}).length;

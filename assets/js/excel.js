...
var rpaPlanCount = table.rows().data().filter(function (row) {
    return row.customized_level2_status === 'Plan' && row.type === 'EXCEL';
}).length;

var rpaAcceptCount = table.rows().data().filter(function (row) {
    return row.customized_level2_status === 'Accept' && row.type === 'EXCEL';
}).length;

var rpaOngoingCount = table.rows().data().filter(function (row) {
    return row.customized_level2_status === 'Ongoing' && row.type === 'EXCEL';
}).length;

...
class Rejected extends CI_Controller {
    ...
    public function index(){
        ...
        if($this->session->userdata('user_wasp')){
            ...
            $data['areas'] = $this->rejected_model->get_areas();
            
            // Load the areas view with data
            $this->load->view('Rejected/index', $data);
        }
        else{
    ...
    public function get_areas() {
        ...
        $data = $this->rejected_model->get_areas($division, $date_ot);
        echo json_encode(['data' => $data]);
    }

    public function get_area_details($id) {
        $area_details = $this->rejected_model->get_area_details($id);

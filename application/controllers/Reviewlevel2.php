...
class Reviewlevel2 extends CI_Controller {
    public function __construct() {
        parent::__construct();
        $this->load->library('session');
        $this->load->helper('url'); // Load URL Helper
        $this->load->model('reviewlevel2_model');
    ...
    public function index(){
        ...
        if($this->session->userdata('user_wasp')){
            ...
            $data['areas'] = $this->reviewlevel2_model->get_areas();
            
            // Load the areas view with data
            $this->load->view('Reviewlevel2/index', $data);
        }
        else{
    ...
    public function get_area_details($id) {
        ...
        $filtered_area_details = $this->reviewlevel2_model->get_area_details_by_control_number($control_number);
    ...
    public function update_status() {
        ...
        foreach ($status_updates as $update) {
            ...
            $this->reviewlevel2_model->update_status($id, $status, $remarks, $date_add, $fullname);
        }

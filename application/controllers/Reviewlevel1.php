...
class Reviewlevel1 extends CI_Controller {
    public function __construct() {
        parent::__construct();
        $this->load->library('session');
        $this->load->helper('url'); // Load URL Helper
        $this->load->model('reviewlevel1_model');
    ...
    public function index(){
        ...
        if($this->session->userdata('user_wasp')){
            ...
            $data['areas'] = $this->reviewlevel1_model->get_areas();
            
            // Load the areas view with data
            $this->load->view('Reviewlevel1/index', $data);
        }
        else{
    ...
    public function get_area_details($id) {
        ...
        $filtered_area_details = $this->reviewlevel1_model->get_area_details_by_control_number($control_number);
    ...
    public function update_status() {
        ...
        foreach ($status_updates as $update) {
            ...
            $this->reviewlevel1_model->update_status($id, $status, $remarks, $date_add, $fullname);
        }

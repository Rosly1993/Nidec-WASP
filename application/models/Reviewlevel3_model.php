<?php
class Reviewlevel3_model extends CI_Model {
	...
	public function get_areas() {
        ...
        $this->db->where('status', 'Level2 Approved');
    ...
    public function get_area_details_by_control_number($control_number) {
        ...
        $query = $this->db->get_where('tbl_request', ['control_number' => $control_number, 'status' => 'Level2 Approved']);
    ...
    public function update_status($id, $status, $remarks, $date_add, $fullname) {
        ...
        $this->db->where('id', $id);
        $this->db->update('tbl_request', array('status' => $status, 'remarks' => $remarks, 'level3_date' => $date_add, 'level3_approver' => $fullname));
    }
